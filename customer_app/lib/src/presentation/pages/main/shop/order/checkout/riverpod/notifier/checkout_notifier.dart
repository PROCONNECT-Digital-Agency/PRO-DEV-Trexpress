...
class CheckoutNotifier extends StateNotifier<CheckoutState> {
  ...
  void changeDeliveryType(DeliveryTypeEnum deliveryType, {ShopData? shop}) {
    ...
    state = state.copyWith(shippingDataList: newList , deliveryFee: shop?.price?.toDouble() ?? 0 , deliveryTypeId: shop?.id ?? 1);
        ...
        for (final shipping in shippingList) {
          if (shipping.shopId == shopTotal.shopData.id) {
            deliveryAddressId =
                shipping.activeDelivery == DeliveryTypeEnum.delivery
                    ? LocalStorage.instance
                        .getNewAddresses()[state.addressIndex]
                        .id
                    : null;
            ...
            deliveryTime = shipping.activeDelivery == DeliveryTypeEnum.delivery
                ? state.deliveryTime
                : null;
            deliveryType =  shipping.activeDelivery == DeliveryTypeEnum.delivery ? 'delivery' : 'pickup';
            break;
          }
        }
        shopsOrder.add(
  ...
  Future<void> checkCoupon(
    ...
    if (connected) {
      ...
      response.when(
        success: (data) async {
          state = state.copyWith(isCheckingCoupon: false , isApplied : true , isError : false);
        ...
        failure: (failure) {
          state = state.copyWith(isCheckingCoupon: false , isApplied : false, isError : true);
